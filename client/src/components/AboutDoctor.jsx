import { useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { FaCheckCircle, FaAward, FaUserMd, FaGraduationCap } from 'react-icons/fa';

const specialInterests = [
  { label: 'Growth & Development',           color: 'bg-primary-50 text-primary-green border-primary-100' },
  { label: 'General Paediatrics (0–12 Years)', color: 'bg-primary-50 text-primary-700 border-primary-100' },
  { label: 'Behavioural Issues in Children', color: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
  { label: 'Neonatology & Newborn Care',     color: 'bg-sky-50 text-sky-700 border-sky-100' },
  { label: 'Child Health Education & Vlogging', color: 'bg-amber-50 text-amber-700 border-amber-100' },
  { label: 'Life Philosophy & Wellness',     color: 'bg-emerald-50 text-emerald-700 border-emerald-100' },
];

const credentials = [
  { icon: FaAward,          text: '15+ Years Experience in Paediatrics & Neonatology' },
  { icon: FaGraduationCap,  text: 'MBBS & DCH (JNMC, AMU) | Hospital Management' },
  { icon: FaUserMd,         text: 'Ex-Max Hospitals, St Stephens & PGICH Noida' },
];

const AboutDoctor = () => {
  const [isMobile, setIsMobile] = useState(false);

  useEffect(() => {
    const handleResize = () => setIsMobile(window.innerWidth < 1024);
    handleResize();
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, []);

  return (
    <section
      id="about"
      className="py-16 sm:py-20 lg:py-24 relative flex items-center overflow-hidden"
      style={{ isolation: 'isolate' }}
    >
      {/* ── Static Light Green/White Gradient Background ── */}
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(160deg, #ffffff 0%, #f0fbf4 40%, #e8f5e9 100%)',
          zIndex: 0,
        }}
      />

      {/* Subtle decorative blob */}
      <div
        style={{
          position: 'absolute', top: 0, right: 0,
          width: '320px', height: '320px',
          background: 'rgba(240,251,244,1)',
          borderRadius: '50%', filter: 'blur(60px)',
          transform: 'translate(33%, -50%)',
          pointerEvents: 'none', zIndex: 2,
        }}
      />

      {/* ── All content ── */}
      <div className="section-container" style={{ position: 'relative', zIndex: 10 }}>

        {/* Section Header */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
          className="text-center mb-8 sm:mb-12"
        >
          <span className="section-tag mb-2">Meet The Doctor</span>
          <h2 className="section-heading mb-2 text-2xl sm:text-3xl lg:text-4xl">About Dr. S. Mashhood Abbas</h2>
          <div className="section-divider mb-3" />
          <p className="section-subheading text-xs sm:text-sm max-w-2xl mx-auto">
            Dedicated to providing exceptional care for children of all ages with compassion and expertise
          </p>
        </motion.div>

        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">

          {/* ── Left: Doctor Image ── */}
          <motion.div
            initial={{ opacity: 0, x: -40 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.65 }}
            className="relative flex justify-center order-1 lg:order-1"
          >
            <div className="relative w-full max-w-[340px]">
              {/* Background accent */}
              <div className="absolute inset-4 bg-primary-50 rounded-3xl rotate-3" />

              {/* Card */}
              <div className="relative bg-white rounded-3xl shadow-card-lg border border-neutral-100 overflow-hidden transition-transform duration-500">

                {/* Image */}
                <div className="w-full h-80 sm:h-[380px] relative bg-neutral-100">
                  <img
                    src="https://res.cloudinary.com/dxqn9ka7p/image/upload/v1785326786/WhatsApp_Image_2026-07-29_at_10.40.56_AM_mpfwsy.jpg"
                    alt="Dr. S. Mashhood Abbas — Pediatrician & Neonatologist"
                    className="w-full h-full object-cover"
                    style={{ display: 'block', objectPosition: 'center 5%' }}
                  />
                  {/* Gradient at bottom for name readability */}
                  <div className="absolute bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-black/50 to-transparent" />
                  {/* Name overlay */}
                  <div className="absolute bottom-4 left-0 right-0 text-center px-3">
                    <p className="font-poppins font-semibold text-white text-sm drop-shadow-md">Dr. S. Mashhood Abbas</p>
                    <p className="text-[11px] text-white/85 drop-shadow">Pediatrician &amp; Neonatologist</p>
                  </div>
                </div>

                {/* Credentials chip */}
                <div className="flex items-center justify-center gap-2 px-4 py-3 flex-wrap">
                  <span className="inline-flex items-center gap-1.5 bg-primary-50 text-primary-green text-xs font-semibold border border-primary-100 px-3 py-1.5 rounded-full">
                    <FaCheckCircle className="text-xs" /> Certified Pediatrician
                  </span>
                  <span className="inline-flex items-center gap-1.5 bg-primary-50 text-primary-light text-xs font-semibold border border-primary-100 px-3 py-1.5 rounded-full">
                    <FaCheckCircle className="text-xs" /> Neonatologist
                  </span>
                </div>
              </div>
            </div>
          </motion.div>

          {/* ── Right: Bio ── */}
          <motion.div
            initial={{ opacity: 0, x: 40 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.65, delay: 0.1 }}
            className="order-2 lg:order-2"
          >
            {!isMobile && (
              <>
                <h3 className="text-xl sm:text-2xl font-poppins font-bold text-neutral-900 mb-1">Dr. S. Mashhood Abbas</h3>
                <p className="text-primary-green font-semibold text-sm mb-4">Pediatrician &amp; Neonatologist</p>
              </>
            )}

            {/* Credential badges */}
            <div className="space-y-2 mb-5">
              {credentials.map((c, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <div className="w-8 h-8 bg-primary-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    <c.icon className="text-primary-green text-xs" />
                  </div>
                  <span className="text-xs sm:text-sm text-neutral-700 font-medium">{c.text}</span>
                </div>
              ))}
            </div>

            {/* Bio paragraphs */}
            <div className="space-y-3 text-neutral-600 text-xs sm:text-sm leading-relaxed mb-5">
              <p>
                <strong>Dr S. Mashhood Abbas</strong>, is a consultant with 15 years of experience in the field of Paediatrics and Neonatology and has worked in some of the most reputed institutions of the country including Max Hospitals, St Stephens Hospital and Post Graduate Institute of Child Health, Noida.
              </p>
              <p>
                He has done his MBBS and DCH from JNMC, Aligarh Muslim University and is also pursuing a course in Hospital Management.
              </p>
              <p>
                Dr Syed is well versed in handling all patients from 0 to 12 years with various ailments and has special interest in General Paediatrics especially Growth and Development and Behavioural issues in Children.
              </p>
              <p>
                He is also a Vlogger about recent trends in health of children and also writes about life philosophy to help people think and live better.
              </p>
            </div>

            {/* Special Interests */}
            <div className="mb-6">
              <h4 className="text-xs sm:text-sm font-poppins font-semibold text-neutral-900 mb-2">Special Interests</h4>
              <div className="flex flex-wrap gap-1.5">
                {specialInterests.map((item, i) => (
                  <span
                    key={i}
                    className={`text-[10px] sm:text-xs font-medium border rounded-full px-2.5 py-1 ${item.color}`}
                  >
                    {item.label}
                  </span>
                ))}
              </div>
            </div>

            <motion.button
              onClick={() => {
                const el = document.querySelector('#forms');
                if (el) el.scrollIntoView({ behavior: 'smooth' });
              }}
              className="btn-primary text-xs sm:text-sm px-5 py-2.5"
              whileHover={{ scale: 1.03, y: -2 }}
              whileTap={{ scale: 0.97 }}
            >
              Book a Consultation
            </motion.button>
          </motion.div>
        </div>
      </div>
    </section>
  );
};

export default AboutDoctor;
